  ...
  <div class="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
    ...
    <div class="pt-20 px-4 sm:px-6 lg:px-8">
      <div class="max-w-7xl mx-auto">
        <!-- 页面标题 -->
        <div class="mb-8">
          <h1 class="text-3xl font-bold text-gray-900 mb-2">{{ levelInfo.name }}</h1>
        ...
        <!-- 诗词列表 -->
        <div v-if="poems.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          <div v-for="poem in poems"
            ...
            <!-- 封面区域 -->
            <div class="relative bg-gradient-to-br from-blue-500 to-cyan-600 p-6" style="min-height: 160px;">
              <div class="flex flex-col items-center justify-center text-white h-full">
                <h3 class="text-2xl font-kai mb-2">{{ poem.title }}</h3>
                <p class="text-white/80 text-sm">{{ poem.dynasty }} · {{ poem.author }}</p>
              </div>
            </div>
            
            <!-- 内容预览 -->
            <div class="p-4">
              <div class="text-gray-600 text-sm mb-3 font-song">
                <p class="mb-1">{{ poem.content[0] }}</p>
                <p>{{ poem.content[1] }}</p>
              </div>
        ...
        <div v-else class="text-center py-20">
          <div class="text-gray-500">
            <div class="text-xl mb-2">暂无诗词</div>
            <div class="text-sm">该级别下还没有添加诗词内容</div>
          </div>
        </div>
      </div>
    </div>
